...
Now you should access CLI script: $ backend_api --help
Alternatively you can access CLI script via poetry run: $ poetry run backend_api --help
To deactivate virtualenv simply type: $ deactivate
To activate shell completion:
 - for bash: $ echo 'eval "$(_BACKEND_API_COMPLETE=source_bash backend_api)' >> ~/.bashrc
 - for zsh: $ echo 'eval "$(_BACKEND_API_COMPLETE=source_zsh backend_api)' >> ~/.zshrc
 - for fish: $ echo 'eval "$(_BACKEND_API_COMPLETE=source_fish backend_api)' >> ~/.config/fish/completions/backend_api.fish
EOF